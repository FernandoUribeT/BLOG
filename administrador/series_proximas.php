...
switch ($accion) {
    case "Agregar":
        $sentenciaSQL = $conexion->prepare("INSERT INTO series_proximas (titulo, comentario, imagen, trailer) VALUES (:titulo, :comentario, :imagen, :trailer);");
        ...
        header("Location: series_proximas.php");
        break;

    case "Modificar":
        $sentenciaSQL = $conexion->prepare("UPDATE series_proximas SET titulo=:titulo, comentario=:comentario, trailer=:trailer WHERE id=:id");
        $sentenciaSQL->bindParam(':titulo', $txtTitulo);
        $sentenciaSQL->bindParam(':comentario', $txtComentario);
        $sentenciaSQL->bindParam(':trailer', $trailer);
        ...
        if ($txtImagen != "") {
            ...
            $tmpImagen = $_FILES["txtImagen"]["tmp_name"];
            move_uploaded_file($tmpImagen, "../assets/" . $nombreArchivo);
            $sentenciaSQL = $conexion->prepare("SELECT imagen FROM series_proximas WHERE id=:id");
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
            $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);

            $sentenciaSQL = $conexion->prepare("UPDATE series_proximas SET imagen=:imagen WHERE id=:id");
            $sentenciaSQL->bindParam(':imagen', $nombreArchivo);
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
        }
    ...
    case "Seleccionar":
        $sentenciaSQL = $conexion->prepare("SELECT * FROM series_proximas WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
    ...
    case "Borrar":
        $sentenciaSQL = $conexion->prepare("DELETE FROM series_proximas WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        header("Location: series_proximas.php");
        break;
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM series_proximas ORDER by id desc");
$sentenciaSQL->execute();
