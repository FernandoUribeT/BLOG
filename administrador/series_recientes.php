...
switch ($accion) {
    ...
    case "Agregar":
        $sentenciaSQL = $conexion->prepare("INSERT INTO series_recientes (titulo, comentario, imagen, netflix, hbomax, primevideo, disney, star, crunchyroll, trailer) VALUES (:titulo, :comentario, :imagen, :netflix, :hbomax, :primevideo, :disney, :star, :crunchyroll, :trailer);");
        ...
        header("Location: series_recientes.php");
        break;

    case "Modificar":
        $sentenciaSQL = $conexion->prepare("UPDATE series_recientes SET titulo=:titulo, comentario=:comentario, netflix=:netflix, hbomax=:hbomax, primevideo=:primevideo, disney=:disney, star=:star, crunchyroll=:crunchyroll, trailer=:trailer WHERE id=:id");
        $sentenciaSQL->bindParam(':titulo', $txtTitulo);
        $sentenciaSQL->bindParam(':comentario', $txtComentario);
        $sentenciaSQL->bindParam(':netflix', $netflix);
        $sentenciaSQL->bindParam(':hbomax', $hbomax);
        ...
        if ($txtImagen != "") {
            ...
            $tmpImagen = $_FILES["txtImagen"]["tmp_name"];
            move_uploaded_file($tmpImagen, "../assets/" . $nombreArchivo);
            $sentenciaSQL = $conexion->prepare("SELECT imagen FROM series_recientes WHERE id=:id");
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
            $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);

            $sentenciaSQL = $conexion->prepare("UPDATE series_recientes SET imagen=:imagen WHERE id=:id");
            $sentenciaSQL->bindParam(':imagen', $nombreArchivo);
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
        }
    ...
    case "Seleccionar":
        $sentenciaSQL = $conexion->prepare("SELECT * FROM series_recientes WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
    ...
    case "Borrar":
        $sentenciaSQL = $conexion->prepare("DELETE FROM series_recientes WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        header("Location: series_recientes.php");
        break;
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM series_recientes ORDER by id desc");
$sentenciaSQL->execute();
