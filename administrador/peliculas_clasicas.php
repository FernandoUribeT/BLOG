...
switch ($accion) {
    ...
    case "Agregar":
        $sentenciaSQL = $conexion->prepare("INSERT INTO peliculas_clasicas (titulo, comentario, imagen, netflix, hbomax, primevideo, disney, star, trailer) VALUES (:titulo, :comentario, :imagen, :netflix, :hbomax, :primevideo, :disney, :star, :trailer);");
        ...
        header("Location: peliculas_clasicas.php");
        break;

    case "Modificar":
        $sentenciaSQL = $conexion->prepare("UPDATE peliculas_clasicas SET titulo=:titulo, comentario=:comentario, netflix=:netflix, hbomax=:hbomax, primevideo=:primevideo, disney=:disney, star=:star, trailer=:trailer WHERE id=:id");
        $sentenciaSQL->bindParam(':titulo', $txtTitulo);
        $sentenciaSQL->bindParam(':comentario', $txtComentario);
        $sentenciaSQL->bindParam(':netflix', $netflix);
        $sentenciaSQL->bindParam(':hbomax', $hbomax);
        ...
        if ($txtImagen != "") {
            ...
            $tmpImagen = $_FILES["txtImagen"]["tmp_name"];
            move_uploaded_file($tmpImagen, "../assets/" . $nombreArchivo);
            $sentenciaSQL = $conexion->prepare("SELECT imagen FROM peliculas_clasicas WHERE id=:id");
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
            $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);

            $sentenciaSQL = $conexion->prepare("UPDATE peliculas_clasicas SET imagen=:imagen WHERE id=:id");
            $sentenciaSQL->bindParam(':imagen', $nombreArchivo);
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
        }
    ...
    case "Seleccionar":
        $sentenciaSQL = $conexion->prepare("SELECT * FROM peliculas_clasicas WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
    ...
    case "Borrar":
        $sentenciaSQL = $conexion->prepare("DELETE FROM peliculas_clasicas WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        header("Location: peliculas_clasicas.php");
        break;
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM peliculas_clasicas ORDER by id desc");
$sentenciaSQL->execute();
