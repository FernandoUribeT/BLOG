...
switch ($accion) {
    case "Agregar":
        ...
        $sentenciaSQL = $conexion->prepare("INSERT INTO galardonadas (titulo, comentario, imagen, presentacion) VALUES (:titulo, :comentario, :imagen, :presentacion);");
        $sentenciaSQL->bindParam(':titulo', $txtTitulo);
        $sentenciaSQL->bindParam(':comentario', $txtComentario);
        $sentenciaSQL->bindParam(':presentacion', $presentacion);
        ...
        header("Location:galardonadas.php");
        break;


    case "Modificar":
        $sentenciaSQL = $conexion->prepare("UPDATE galardonadas SET titulo=:titulo, comentario=:comentario, presentacion=:presentacion WHERE id=:id");
        $sentenciaSQL->bindParam(':titulo', $txtTitulo);
        $sentenciaSQL->bindParam(':comentario', $txtComentario);
        $sentenciaSQL->bindParam(':presentacion', $presentacion);
        ...
        if ($txtImagen != "") {
            ...
            $tmpImagen = $_FILES["txtImagen"]["tmp_name"];
            move_uploaded_file($tmpImagen, "../assets/" . $nombreArchivo);
            $sentenciaSQL = $conexion->prepare("SELECT imagen FROM galardonadas WHERE id=:id");
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
            $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);

            $sentenciaSQL = $conexion->prepare("UPDATE galardonadas SET imagen=:imagen WHERE id=:id");
            $sentenciaSQL->bindParam(':imagen', $nombreArchivo);
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
        }
    ...
    case "Seleccionar":
        $sentenciaSQL = $conexion->prepare("SELECT * FROM galardonadas WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
    ...
    case "Borrar":
        $sentenciaSQL = $conexion->prepare("DELETE FROM galardonadas WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        header("Location:galardonadas.php");
        break;
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM galardonadas ORDER by id desc");
$sentenciaSQL->execute();
    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label for="presentacion">URL de Presentacion:</label>
                    <input type="text" style="background-color: #18171c; border: 1px solid #044c9c;" class="form-control" value="<?php echo $presentacion; ?>" name="presentacion" id="presentacion" placeholder="URL de la presentacion: ">
                </div>
