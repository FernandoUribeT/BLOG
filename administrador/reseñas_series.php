...
switch ($accion) {
    case "Agregar":
        $sentenciaSQL = $conexion->prepare("INSERT INTO reseñas_series (titulo, comentario, imagen) VALUES (:titulo, :comentario, :imagen);");
        ...
        header("Location: reseñas_series.php");
        break;

    case "Modificar":
        $sentenciaSQL = $conexion->prepare("UPDATE reseñas_series SET titulo=:titulo WHERE id=:id");
        $sentenciaSQL->bindParam(':titulo', $txtTitulo);
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $sentenciaSQL = $conexion->prepare("UPDATE reseñas_series SET comentario=:comentario WHERE id=:id");
        $sentenciaSQL->bindParam(':comentario', $txtComentario);
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        ...
        if ($txtImagen != "") {
            ...
            $tmpImagen = $_FILES["txtImagen"]["tmp_name"];
            move_uploaded_file($tmpImagen, "../assets/" . $nombreArchivo);
            $sentenciaSQL = $conexion->prepare("SELECT imagen FROM reseñas_series WHERE id=:id");
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
            $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);

            $sentenciaSQL = $conexion->prepare("UPDATE reseñas_series SET imagen=:imagen WHERE id=:id");
            $sentenciaSQL->bindParam(':imagen', $nombreArchivo);
            $sentenciaSQL->bindParam(':id', $txtID);
            $sentenciaSQL->execute();
        }
    ...
    case "Seleccionar":
        $sentenciaSQL = $conexion->prepare("SELECT * FROM reseñas_series WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        $publicaciones = $sentenciaSQL->fetch(PDO::FETCH_LAZY);
    ...
    case "Borrar":
        $sentenciaSQL = $conexion->prepare("DELETE FROM reseñas_series WHERE id=:id");
        $sentenciaSQL->bindParam(':id', $txtID);
        $sentenciaSQL->execute();
        header("Location: reseñas_series.php");
        break;
...
$sentenciaSQL = $conexion->prepare("SELECT * FROM reseñas_series ORDER by id desc");
$sentenciaSQL->execute();
