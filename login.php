...
if (isset($_POST["login"])) {
  ...
  $result = mysqli_query($conn, "SELECT * FROM usuarios WHERE BINARY username = '$user'");
  $row = mysqli_fetch_assoc($result);
  if (mysqli_num_rows($result) > 0) {
    if (password_verify($password, $row["password"])) {
      $_SESSION["login"] = true;
      $_SESSION["id"] = $row["id"];
      if ($row["rol_id"] == 1 or $row["rol_id"] == 2) { //aqui si rol_id=2 es administrador, rol_id = 1 es el dueño
        echo '<script language="javascript">window.location="/BLOG/administrador/inicio.php";</script>';
      }
      if ($row["rol_id"] == 3) { //aqui es visitante con cuenta
...
if (isset($_POST["cinehub"])) { //aqui es un usuario sin cuenta
  unset(
    $_SESSION["id"],
  );
